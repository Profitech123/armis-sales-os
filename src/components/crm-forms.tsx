...
export function ContactEditForm({ contact, accounts, editable }: { contact: { id:string; account_id:string; first_name:string; last_name:string; email:string|null; phone:string|null; job_title:string|null; relationship_role:string|null; archived_at:string|null; record_version:number }; accounts: {id:string;name:string}[]; editable:boolean }) {
  ...
  return <form className="card form-grid form-grid-wide" action={action} noValidate><h2>Contact details</h2><Feedback state={state} /><input type="hidden" name="id" value={contact.id} /><input type="hidden" name="version" value={contact.record_version} /><label>Account<select name="accountId" defaultValue={contact.account_id}>{accounts.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select></label><label>First name<input name="firstName" required defaultValue={contact.first_name} aria-invalid={Boolean(state.fieldErrors?.firstName)} aria-describedby={describedBy(state,"firstName")} /><FieldError state={state} name="firstName" /></label><label>Last name<input name="lastName" defaultValue={contact.last_name} /></label><label>Email<input name="email" type="email" defaultValue={contact.email ?? ""} aria-invalid={Boolean(state.fieldErrors?.email)} aria-describedby={describedBy(state,"email")} /><FieldError state={state} name="email" /></label><label>Phone<input name="phone" defaultValue={contact.phone ?? ""} /></label><label>Job title<input name="jobTitle" defaultValue={contact.job_title ?? ""} /></label><label>Relationship role<input name="relationshipRole" defaultValue={contact.relationship_role ?? ""} /></label><label>Status<select name="archived" defaultValue={contact.archived_at ? "true" : "false"}><option value="false">Active</option><option value="true">Archived</option></select></label><button className="button dark" disabled={pending}>{pending ? "Saving…" : "Save contact"}</button></form>;
}
